...
- **均線由日線計算**（年線需 240 根日線，故須先執行 `run_ingestion.py` 累積歷史）；
  ...
  以 30 根日線算出的「年線」是誤導。
- **與看盤軟體對不起來是正常的**。本專案的均線以**還原股價**計算（除權息與股票
  分割回溯調整），多數看盤軟體顯示未還原價。以 FinMind 未還原序列交叉驗證的結果：
  週線（5 日）完全吻合，長天期線的落差隨配息率單調上升——2330 年線 −0.12%、
  0050 −0.78%、00878 −2.67%、00919 −5.10%；00631L 因股票分割，半年／年線落差
  達 −80%/−90%。週期慣例也有差（本專案用 20/60/120/240，部分券商用 21/62/124/248）。
- **儀表板與推播的均線可能略有差距**：儀表板含最新一根日線（回答「現在」），
  推播只用已收盤日線（盤中執行時當日尚未收盤）。這是刻意的時基差異。
