    ...
    if is_portfolio:
        ...
        with st.expander("均線現況（週／月／季／半年／年線）", expanded=False):
            try:
                ...
                # 本表與推播的時基**刻意不同**，數值因此會有小差距：本表含最新一根
                # 日線（回答「現在」），推播排除與比較價同日的那根（FR-002，盤中
                # 執行時當日尚未收盤）。不寫出來的話，同一天兩處數字不一致會被
                # 誤讀為其中一邊算錯。
                st.caption(
                    "與推播訊息的均線可能略有差距：本表含最新一根日線，"
                    "推播則只用**已收盤**日線（排除當日）。另外，本專案的均線以"
                    "**還原股價**計算，看盤軟體若顯示未還原價，長天期的線（半年／年）"
                    "會有可觀落差——除權息與股票分割愈多，差距愈大。"
                )
            except Exception as e:
