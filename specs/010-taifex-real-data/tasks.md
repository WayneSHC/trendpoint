...
1. `pytest -m network -q` 綠（1 passed）。~~過程中抓到**雙語表頭**問題：TAIFEX 同一
   ...
   > **更正（2026-07-18，見下方「雙語表頭修正」）**：本點原記載不實，已加刪除線。
   > 查證結果：(a) `4479cd5` 不存在於任何分支歷史；(b) 當時 parser **並未**落地雙語
   > 映射，`_COL_MAP` 全為中文鍵；(c) 當日實測回英文表頭的是 **OpenAPI 當日端點**
   > （`/v1/DailyMarketReportFut`），`futDataDown` 仍回中文（`charset=MS950`）。
   >
   > **時序（推斷，非直接觀測）**：network 測試自 `885d81f`（T008–T012，早於 T017）
   > 起即呼叫 `fetch_latest`，**有**涵蓋 OpenAPI 路徑。故若 T017 之「1 passed」屬實，
   > 當時 OpenAPI 必仍回中文鍵（與 `885d81f` 的 docstring「JSON，中文欄位同 CSV
   > header」一致），意味格式是在 T017 驗收（commit `49c3553`，10:03）與當日稍晚監控
   > 失敗之間變動的——**並非測試未涵蓋該情境**。
   >
   > **無法查證之處（不臆測）**：原註記自述曾看到英文欄位（`Last`），卻未落地修正而
   > 測試仍綠；除非該回應為間歇性，兩者無法同時為真。事後已無從還原當時回應，
   > 故僅記錄「格式變動」與「間歇回應」兩種可能，不擇一斷定。
