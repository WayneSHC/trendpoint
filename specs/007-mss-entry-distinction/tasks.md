...
**Organization**: 依 user story 分組。因訊號→進場→FVG 驗證天然相依，US 順序為序列鏈（非完全獨立），已於 Dependencies 誠實標註。**長側先行**；短側進場執行任務原標 `[BLOCKED-003]`——2026-07-16 已由 spec 003（台指期做空）解封實作。
...
- [x] T017 [US2] [BLOCKED-003] `backtester.py` / `portfolio_backtester.py`：短側反轉進場執行（`direction==−1`）——依賴 spec 003 的 `check_entry_signal` 方向泛化與做空部位管理；003（台指期做空）已於 2026-07-16 落地：`direction==−1` 之空方反轉進場已在期貨路徑實作（enable_short 閘門，見 specs/003-short-side）
...
>
> **補驗更新（2026-07-18）**：上述「待更長日內資料再驗」之條件已於真實 TXF 連續序列
> （6,947 根，為原最長現貨序列的 2.9 倍）驗證並**證偽**——資料長度非瓶頸，MSS 反轉進場
> 在本參數組下具內生稀疏性（FVG on 時 0 筆 / off 時 1 筆）。SC-003 維持未達成，再驗條件
> 改寫為「能提高反轉進場密度的市況或參數組」。同時補齊 SC-002 的雙向驗證（TXF 上零 delta）。
> 詳見 `baseline-pre-mss.md`「2026-07-18 補驗」段。
