...
- [x] T006 [P] [US1] `tests/test_trading_costs.py`：FuturesCostModel 錨定例——TX 1 口 @20,000 單邊 = 20 + 80 = 100 NT$；滑價 = 成交價 ±1 tick 點偏移（**不重複計費**）；MTX/TMF 縮放；來回兩邊各收（SC-002）
- [x] T007 [P] [US1] `tests/test_futures_backtest_e2e.py`：mock TXF → 回測（顯式注入常數 1 口 sizer + FuturesCostModel）→ 跑通、不拋 `FuturesBacktestNotSupportedError`、總摩擦成本 > 0、交易全為多方（SC-004 初步 + SC-006）
- [x] T008 [P] [US1] 改寫 `tests/test_futures_backtest_guard.py`：語意反轉——**單標的**路徑（引擎 + `run_backtest.py`）futures 不再被拒；**組合路徑（portfolio）拒絕斷言保留**（analyze H1：範圍邊界護欄）；equity 預設路徑不受影響（SC-006）

### Implementation for User Story 1

- [x] T009 [US1] 於 `trading_costs.py` 實作 `FuturesCostModel`（建構自 ContractSpec + FuturesCostConfig；slip 點偏移、entry/exit = 定額 + 名目×稅率）；工廠 futures 分支（sizer 部分暫 stub，US2 完成）
- [x] T010 [US1] `backtester.py` 注入改造：`run_backtest(..., cost_model=None, sizer=None, point_value=1.0)`——None 預設現股元件（**既有呼叫零改動**）；232/294/323 行滑價、250/304-305/327-328 行成本、235-236/298 行 sizing/partial 全改走元件；P&L 統一 `units × Δprice × point_value`；**sizer.size 的 price 輸入語意按資產類別**（analyze M1：equity 傳成交價＝現行語意、futures 傳訊號根收盤＝FR-004，docstring 明確化）；改造後立即跑全套既有測試（parity 檢查點）
- [x] T011 [US1] 護欄退役（**僅單標的路徑**，analyze H1）：`backtester.py` 移除引擎層 `assert_backtestable` 拒絕呼叫（函式與例外**定義保留**，import 相容）；`run_backtest.py` 入口放行 futures；**`run_portfolio_backtest.py`（:51）與 `portfolio_backtester.py`（:223）護欄保留**——組合期貨接入不在本 spec，放行會落入現股成本路徑違反憲章 II（D9 修訂）
- [x] T012 [US1] `portfolio_backtester.py` 同步注入（引擎方法同簽名、預設現股元件、既有行為不變）

**Checkpoint**: US1 完成 → SC-002（成本數學）+ SC-006（護欄退役）+ SC-004（初步，常數口）。

> **US1/US2 實作註記（2026-07-16）**：引擎注入（T010）與會計擴充（T017）同屬 backtester.py
> 一次連貫手術完成（避免同檔兩次大改）；期貨元件（T009/T016）於 Foundational 隨骨架一併落地、
> 由先寫的錨定測試（spec 手算值）驗證——TDD「先 FAIL」步驟被單 session 連續實作取代，如實記錄。
> T012 為 no-code-change by design：組合路徑護欄保留（analyze H1）、組合成本邏輯未觸碰，
> 由 test_portfolio_engine_still_rejects_futures 釘住。
> 引擎注入後 parity 預跑：pytest 132 綠 + run_backtest 5 檔數字與基線逐位元一致。
...
- [x] T013 [P] [US2] `tests/test_trading_costs.py` 擴充：FuturesSizer 錨定例——equity 1,000,000 / close 20,000 / TX / rate .055 / util .5 → 2 口；equity 200,000 → 0 口不進場；`partial_units(1,.5)`=0、`(3,.5)`=1（SC-003 / FR-012）
- [x] T014 [P] [US2] `tests/test_futures_backtest_e2e.py` 擴充：工廠分派 margin sizer 之 TXF+MTX e2e——全程口數非負整數、權益曲線無 NaN、佔用保證金 ≤ 權益×使用率；爆倉情境用**測試內自製極端下跌 K 線直接餵引擎**（analyze M2：mock adapter 為固定向上序列，不依賴之）→ 當根強制結清、曲線截止、`summary` 標記（SC-004 完整 / FR-011）
- [x] T015 [P] [US2] `tests/test_lookahead_bias.py` 擴充：期貨防線——截斷第 N 根之後資料不改變第 N 根 sizing 決策（口數）與成交價；成交於 N+1 開盤 ± 滑價 tick（SC-005 / FR-007）

### Implementation for User Story 2

- [x] T016 [US2] 於 `trading_costs.py` 實作 `FuturesSizer`（margin_rate/margin_utilization/floor；partial floor + 0 口語意）；工廠 futures 分支完成
- [x] T017 [US2] `backtester.py` 會計擴充：持倉 mark-to-market 權益、爆倉檢查（權益 ≤ 0 當根強制結清 + 終止 + `summary["blown_up"]`）、交易紀錄擴充（口數、佔用保證金、成本明細分列）；partial-exit 0 口時跳過平倉但止損照移保本位（FR-012 引擎側）
- [x] T018 [US2] `run_backtest.py` 入口穿線：futures instrument 經 `for_asset_class` 分派元件、以既有迴圈跑 config 內 mock TXF/MTX；期貨 summary 顯示口數/保證金資訊
...
- [x] T019 [P] [US3] `tests/test_trading_costs.py` 擴充：Equity 元件 contract test——對隨機 (price, units, equity) 樣本，EquityCostModel/EquitySizer 輸出與現行內聯公式逐位元相同（SC-001 支柱）

### Implementation for User Story 3

- [x] T020 [US3] 回歸驗證：全套 `pytest -q` 綠 + `python run_backtest.py` 4 檔數字與 `baseline-check.md`（T001）逐位元相同（**SC-001 硬關卡**）；任何 diff → 修正引擎注入直至歸零
...
- [x] T021 [P] SoT 稽核：quickstart V6 grep——引擎/元件原始碼無硬編碼期貨費率常數（權威值僅存在 config.yaml 與測試錨定值）；config 載入後 Pydantic 驗證通過（SC-007）
- [x] T022 [P] 執行 `quickstart.md` V1–V6，確認 SC↔測試對照全數成立
- [x] T023 [P] 更新 `CLAUDE.md` 專案地圖（008b 實作狀態、`trading_costs.py` 模組、003 解除阻塞）
- [x] T024 最終 `pytest -q` 全綠（合併關卡）；本 spec 不動訊號邏輯——若實作中觸碰訊號面，附前後回測對照
...
24/24 任務完成。最終驗證：
- **pytest 133 綠**（114 原有 + 19 新；含 spec 004 parity）
- **SC-001**：`run_backtest.py` 現貨 5 檔（2330/0050/00878/00919/00631L）數字與
  `baseline-check.md` **逐位元一致**（引擎注入 + 入口改造後皆驗證）
- **SC-004 CLI 實跑**：mock TXF（4 進場、max 3 口、保證金 59.2 萬）、MTX（1 進場、10 口、
  47.0 萬）照跑無拋錯；組合路徑護欄保留（H1）
- **SC-007 稽核**：引擎/元件/入口零費率硬編碼（權威值僅存 config + 測試錨定）
- quickstart V1–V6 全數成立；契約文件 as-built 同步（TradeCosts 明細精修）
- 本 spec 未觸碰訊號邏輯（無前後回測對照義務；現貨數字位元不變為更強保證）
